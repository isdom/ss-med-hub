 ...
 * @author freeman
 * @since 2021.0.1.0
 * revert spring.config.import https://github.com/alibaba/spring-cloud-alibaba/issues/2833
 */
public class NacosConfigDataLoader implements ConfigDataLoader<NacosConfigDataResource> {
    ...
    public ConfigData doLoad(ConfigDataLoaderContext context,
        ...
        log.info(String.format(
                "[Fixed spring.config.import] NacosConfigDataLoader:doLoad %s %s", context,
                resource));
        try {
    ...
    private void logLoadInfo(String group, String dataId, String config) {
        if (config != null) {
            log.info(String.format(
                    "[Fixed spring.config.import Nacos Config] Load config[dataId=%s, group=%s] success", dataId,
                    group));
        }
        else {
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format(
                    "[Fixed spring.config.import Nacos Config] config[dataId=%s, group=%s] content: \n%s", dataId,
                    group, config));
        }
    }
